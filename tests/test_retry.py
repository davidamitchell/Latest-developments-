...
# ── Gemini retryDelay handling ───────────────────────────────────────────────

def test_retry_after_delay_reads_gemini_structured_details() -> None:
...
def test_retry_after_delay_reads_gemini_string_representation() -> None:
    exc = RuntimeError("RESOURCE_EXHAUSTED 'retryDelay': '39s' quota exceeded")
    assert _retry_after_delay(exc, base_delay=1.0, attempt=1) == 39.0


def test_retry_after_delay_gemini_double_quoted() -> None:
    exc = RuntimeError('quota exceeded "retryDelay": "120s"')
    assert _retry_after_delay(exc, base_delay=1.0, attempt=1) == 120.0


def test_retry_after_delay_structured_details_takes_priority_over_string() -> None:
    exc = RuntimeError("'retryDelay': '10s'")
    exc.details = [{"retryDelay": "45s"}]  # type: ignore[attr-defined]
    assert _retry_after_delay(exc, base_delay=1.0, attempt=1) == 45.0


def test_with_backoff_uses_gemini_retry_delay() -> None:
    """with_backoff() sleeps for the retryDelay when present in the exception."""
    exc = RuntimeError("'retryDelay': '30s' RESOURCE_EXHAUSTED")
    calls = 0
