...
### Fixed
- **Gemini rate limiting (2026-05-04)**: The pipeline was exhausting the 5-RPM free-tier quota immediately by firing all enrichment requests in a tight loop. Two fixes:
  ...
  3. `with_backoff()` in `src/retry.py` now reads the `Retry-After` header from HTTP exceptions (RFC 6585) and uses it as the sleep duration, benefiting all fetchers that use the shared retry helper.
